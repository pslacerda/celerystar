...
validation_dict = {
    ...
    "obj": cs.Object(
        properties={"uni": cs.Union(items=[cs.Integer(), cs.Number()])}
    ),
    "int": cs.Integer(),
}


make_base_build_task_patch = patch.object(
    cs.BaseService, "_build_task", create=True
)
...
def test_base_service_validate_celery_app(_):
    ...
    with raises(cs.ConfigurationError, match="cannot handle result backends"):
        app = cs.Celery(backend="redis://")
        srv = cs.BaseService(
            app, cs.Injector([], {}), dummy_impl, cs.Type(), {}
        )
...
def test_base_service_validate(_):
    ...
    with raises(
        ...
        srv = cs.BaseService(
            cs.Celery(), cs.Injector([], {}), unresolvable, cs.Type(), {}
        )
...
def test_base_service_apply_local(build_task):
    ...
    srv = cs.BaseService(
        cs.Celery(), cs.Injector([], {}), dummy_impl, InitialState, {}
    )
...
def test_resulter_mixin_validate_celery_app(self):
    ...
    with patch.object(cs.ResulterMixin, "app", app, create=True):
        with raises(
            cs.ConfigurationError, match="a result backend is required"
        ):
...
def test_resuler_mixin_apply_error(_a, _b, c):
    ...
    with raises(
        cs.ConfigurationError, match="timeout>0 is required to get results"
    ):
...
@make_service_task_builder_mixin_patch(
    cs.CallableTaskBuilderMixin, CallableImpl()
)
...
def test_service_component():
    ...
    app = cs.Celery()
    srv = cs.make_service(
        service_component_impl, [], InitialState, app, name="a_service"
    )

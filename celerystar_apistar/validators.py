...
from celerystar_apistar import formats
...
class String(Validator):
    ...
    def validate(self, value, definitions=None, allow_coerce=False):
        if value is None and self.allow_null:
            ...
            self.error("null")
        elif self.format in FORMATS and FORMATS[self.format].is_native_type(
            value
        ):
            return value
...
class Object(Validator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        pattern_properties = (
            ...
            else dict_type(pattern_properties)
        )
        required = (
            list(required) if isinstance(required, (list, tuple)) else required
        )
        ...
        assert all(
            isinstance(v, Validator) for v in pattern_properties.values()
        )
        assert additional_properties is None or isinstance(
            additional_properties, (bool, Validator)
        )
    ...
    def validate(self, value, definitions=None, allow_coerce=False):
        ...
        remaining = [
            key for key in value.keys() if key not in set(validated.keys())
        ]
...
class Array(Validator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert additional_items is None or isinstance(
            additional_items, (bool, Validator)
        )
...
class Union(Validator):
    ...
    def __init__(self, items, allow_null=False):
        assert isinstance(items, list) and all(
            isinstance(i, Validator) for i in items
        )
...
class Uniqueness:
    ...
    def make_hashable(self, element):
        ...
        if element is True:
            ...
            return (
                "list",
                tuple([self.make_hashable(item) for item in element]),
            )

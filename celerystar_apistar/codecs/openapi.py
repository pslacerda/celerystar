    ...
    definitions={
        ...
        "Components": validators.Object(
            properties=[
                (
                    "schemas",
                    validators.Object(additional_properties=JSON_SCHEMA),
                )
            ]
        ),
...
class OpenAPICodec(BaseCodec):
    ...
    def get_content(self, openapi, base_url, schema_definitions):
        ...
        for path, path_info in openapi.get("paths", {}).items():
            operations = {
                key: path_info[key] for key in path_info if key in METHODS
            }
    ...
    def get_link(
        ...
        base_url = lookup(
            operation_info, ["servers", 0, "url"], default=base_url
        )
    ...
    def get_operation(self, link, operation_id, tag=None, schema_defs=None):
        ...
        if link.get_body_field():
            ...
            operation["requestBody"] = {
                "content": {link.encoding: content_info}
            }
        return operation

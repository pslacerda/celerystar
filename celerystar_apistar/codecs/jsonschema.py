...
from celerystar_apistar import types, validators
...
JSON_SCHEMA = validators.Object(
    ...
    properties=[
        ...
        (
            "required",
            validators.Array(
                items=validators.String(), min_items=1, unique_items=True
            ),
        ),
        # Array
        (
...
def is_any(type_strings, struct):
    ...
    return (
        len(type_strings) == 6 and not set(struct.keys()) & ALL_PROPERTY_NAMES
    )
...
class JSONSchemaCodec(BaseCodec):
    ...
    def decode(self, bytestring, **options):
        try:
            data = json.loads(
                bytestring.decode("utf-8"), object_pairs_hook=dict_type
            )
    ...
    def encode_to_data_structure(
        self, item, defs=None, def_prefix=None, is_def=False
    ):
            ...
            if item.properties:
                value["properties"] = dict_type(
                    [
                        (
                            key,
                            self.encode_to_data_structure(
                                value, defs, def_prefix
                            ),
                        )
